...
%My helpful predicates
expertiseslist(Expertises):- findall(E, modifiedexpertise(_, E), Exps), setof(Ex, member(Ex, Exps), Expertises).
modifiedexpertise(S,E):- atleastKfriends(S), info(S,_,E).
allstudentslits(Students):- findall(chosen(S), atleastKfriends(S), Students).
greaterstudentslits(S, Students):- findall(chosen(S1), (atleastKfriends(S1), S1 > S), Students).
notfriendslits(S, NotFriends):- mutualfriendslits(S, Friends), greaterstudentslits(S, Students), append([chosen(S)], Friends, Sub), subtract(Students, Sub, NotFriends). 
mutualfriendslits(S, Friends):- findall(chosen(S2), (friends(S,S2), friends(S2,S)), F), subtract(F, [chosen(S)], Friends). %%Checks for possible being a friend of yourself error
atleastKfriends(S):- numMembersTeam(K), student(S), mutualfriendslits(S, Friends), length(Friends, N), K1 is K-1, N>=K1.
...
% each student is a variable, which can be chosen or not, so 25 variables

%%%%%%  2. Clause generation for the SAT solver:

%%STEPS WOULD BE - 
%%% Filter out students without at least K-1 friends
%%% 1- Make sure 6 students are selected
%%% 2- Make sure students who are not friends cant be selected at the same time (only 1 is selected)
%%% 3 - Make sure those with the same expertise do not go together
writeClauses:- 
      exactly6students,       %Team with exactly K students, so K students must be chosen   
      notfriendsnottogether, %Each one considers all the others a friend
      sameexpertisenottogether,      %All students have different expertise topics
      true,!.                 % this way you can comment out ANY previous line of writeClauses
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

exactly6students:- numMembersTeam(K), findall(chosen(S), atleastKfriends(S), Lits), exactly(K, Lits), fail.
exactly6students.

notfriendsnottogether:-atleastKfriends(S), notfriendslits(S,NotFriends), append([chosen(S)], NotFriends, Lits), atMost(1,Lits), fail.
notfriendsnottogether.

sameexpertisenottogether:- expertiseslist(Expertises), member(Exp, Expertises), findall(chosen(S), modifiedexpertise(S,Exp), Lits), atMost(1,Lits), fail.
sameexpertisenottogether.

%%%%%%  3. DisplaySol: show the solution. Here M contains the literals that are true in the model:

%displaySol(M):- nl, writ12016 e(M), nl, nl, fail.
