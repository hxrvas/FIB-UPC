...
%%As many variables as cities

%%%%%%  2. Clause generation for the SAT solver:

writeClauses:- 
      atmostmaxstations,            %At most maxStations 
      gasstationperroad,        %For each road, at least 1 gas station
      true,!.                       % this way you can comment out ANY previous line of writeClauses         
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

atmostmaxstations:- findall(install(C), city(C), Lits), maxStations(N), atMost(N, Lits), fail.
atmostmaxstations.

gasstationperroad:- road(I, J), Lits = [install(I),install(J)], atLeast(1, Lits), fail.
gasstationperroad. 
%%%%%%  3. DisplaySol: show the solution. Here M contains the literals that are true in the model:

%displaySol(M):- nl, write(M), nl, nl, fail.
displaySol(M):- city(C), member(install(C),M), write(C), write(' '), fail.
displaySol(_):- nl.
