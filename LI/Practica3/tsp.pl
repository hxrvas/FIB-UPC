...
citynotone(I):- adjacency(I,_), I =\= 1.
position2(P):- numCities(N), N1 is N-1, between(1,N1,P).

weight(I, J, 1):- city(I), city(J), P1 is I mod 2, P2 is I mod 2, P1 =\= P2.
weight(I, J, 0):- city(I), city(J), P1 is I mod 2, P2 is I mod 2, P1 == P2.
...
writeClauses:- 
   writeClause([visited(1,0)]), numCities(N), writeClause([visited(1,N)]),     %% City 1 is first and last position
   allcitiesexactlyonce,
   allpositionsexactlyonce,
   movebetweenadjacent,
   costs,
   atMostKcosts,
...
movebetweenadjacent:- adjacency(1, L), findall(visited(C1,1), (citynotone(C1), member(C1,L)), NextPs),  
negate(visited(1, 0), NC), writeClause([NC|NextPs]), fail.
      
movebetweenadjacent:- citynotone(C), position2(P), adjacency(C, L), P1 is P+1, position(P1), 
                  findall(visited(C1,P1), (city(C1), member(C1,L)), NextPs),  
                  negate(visited(C, P), NC), writeClause([NC|NextPs]), fail.
movebetweenadjacent.

costs:-
      city(I), city(J), M is I mod 2, M1 is J mod 2,
      M =\= M1, position(P), adjacency(I, L), member(J, L),
      P1 is P+1, position(P1), 
      writeClause([-visited(I,P), -visited(J, P1), cost(P1)]),
      fail.
costs.

atMostKcosts:- maxCost(K), findall(cost(P), position2(P), Lits), atMost(K, Lits), fail.
atMostKcosts.

allcitiesexactlyonce:- citynotone(I), findall(visited(I,P), position(P), Lits), exactly(1, Lits), fail.
allcitiesexactlyonce.

allpositionsexactlyonce:- position(P), findall(visited(I,P), city(I), Lits), exactly(1, Lits), fail.
allpositionsexactlyonce.
